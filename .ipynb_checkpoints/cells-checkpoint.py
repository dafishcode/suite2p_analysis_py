...
#=======================================================================
def fish_find(ogli, fli): # Select which fish data to visualise
#=======================================================================
    ...
    com_trace = list(range(len(fli)))
    com_coord = list(range(len(fli)))
    com_bin = list(range(len(fli)))
    ind_trace = list(range(len(fli * 10)))
    ind_coord = list(range(len(fli * 10)))
    ind_bin = list(range(len(fli * 10)))
     
        
    # Loop through all files in oglist - if you find first element in
    # fish list, then load combined trace/coord + individual trace/coord files into separate lists and increase counter
    #---------------------------------------------------------------------

    counter = 0
    
    for i in range(len(ogli)):
    
        if ogli[i].find('flt_' + fli[counter]) == 0:
            folder = ogli[i]
            com_coord_load = np.load(folder + '/' + fli[counter] + '_all_cord.npy')
            com_trace_load = np.load(folder + '/' + fli[counter] + '_all_sig.npy')
            com_bin_load = np.load(folder + '/' + fli[counter] + '_all_bin.npy')
            com_coord[counter] = com_coord_load
            com_trace[counter] = com_trace_load
            com_bin[counter] = com_bin_load
        
            #for y in range(10):
             #   ind_coord_load = np.load(folder + '/plane_' + str(y) + '_cord.npy')
              #  ind_trace_load = np.load(folder + '/plane_' + str(y) + '_sig.npy')
               # ind_bin_load = np.load(folder + '/plane_' + str(y) + '_bin.npy')
                #ind_coord[counter*10 + y] = ind_coord_load
                #ind_trace[counter*10 + y] = ind_trace_load
               # ind_bin[counter*10 + y] = ind_bin_load
            counter += 1
            print('Loaded ' + ogli[i] + ' (' + str(com_trace_load.shape[1]) + ' frames)')   
            
        if counter == len(fli):
            break 
            
    # create dictionaries containing fish files of all combined traces or cells
    # and then individual plane data - use the names from fli to call them
    #---------------------------------------------------------------------
    ...
    for i in range(len(fli)):
        Fishat.update({fli[i]:com_trace[i]})
        Fishac.update({fli[i]:com_coord[i]})
        Fishbat.update({fli[i]:com_bin[i]})
        
        #for j in range(10):
         #   Fishit.update({'p' + str(j) +'_' + fli[i]:ind_trace[conter+ j]})
          #  Fishic.update({'p' + str(j) +'_' + fli[i]:ind_coord[conter + j]})
           # Fishbit.update({'p' + str(j) +'_' + fli[i]:ind_bin[conter + j]})
        #conter += 10
    
    return(Fishat, Fishac, Fishbat)
